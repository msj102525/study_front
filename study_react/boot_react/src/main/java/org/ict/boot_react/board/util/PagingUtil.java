package org.ict.boot_react.board.util;
...
public class PagingUtil {
    ...
    /**
     * 생성자 1;
     * 1) 현재 페이지 번호 : 1, 한 화면에 출력할 오브젝트 수: 10, 한 화면에 출력할 페이지 번호
     * 수 : 10 으로 기본설정한다.
     * 2) setObjectStartAndEnd()를 호출하여 한 화면에 표시되는 오브젝트의 시작과 마지막을
     * 설정한다.
     */
    public PagingUtil() {
    ...
    /**
     * 생성자 2;
     * 1) '현재 페이지 번호'를 변수로 받는다. 한 화면에 출력할 오브젝트 수: 10, 한 화면에
     * 출력할 페이지 번호 수 : 10 으로 기본설정한다.
     * 2) setObjectStartAndEnd()를 호출하여 한 화면에 표시되는 오브젝트의 시작과 마지막을
     * 설정한다.
     *
    ...
    /**
     * 생성자 3;
     * 1) '현재 페이지 번호','한 화면에 출력할 오브젝트 수','한 화면에 출력할 페이지 번호 수'를
     * 변수로 받는다.
     * 2) setObjectStartAndEnd()를 호출하여 한 화면에 표시되는 오브젝트의 시작과 마지막을
     * 설정한다.
     *
    ...
    public PagingUtil(int currentPageNum, int objectCountPerPage, int pageNumCountPerPage) {
        ...
        this.pageNumCountPerPage = (0 < pageNumCountPerPage) ? pageNumCountPerPage :
                10;
        setObjectStartAndEnd();
    }
    ...
    public boolean setCalcForPaging(Integer objectCountTotal) {
        ...
        try {
            ...
            this.pageNumCountTotal = (int) Math.ceil((double) objectCountTotal /
                    objectCountPerPage);
            int tmpPageNumStart = ((int) Math.ceil(currentPageNum /
                    pageNumCountPerPage)
                    * pageNumCountPerPage);
            int tmpPageNumEnd = 0;
            ...
            this.pageNumEnd = (pageNumCountTotal < tmpPageNumEnd) ?
                    pageNumCountTotal : tmpPageNumEnd;
            this.isPrev = (currentPageNum > pageNumCountPerPage) ? true : false;
            this.isNext = (pageNumEnd < pageNumCountTotal || (pageNumStart <
                    pageNumEnd && currentPageNum < pageNumCountTotal)) ? true : false;
            this.objectEndNum = (objectCountTotal < objectEndNum) ? objectCountTotal :
                    objectEndNum;
            return true;

...
//        boardList.add(boardService.selectBoard(boardNum));
    ...
    @PostMapping("/board") // HTTP POST 요청을 "/boards/board" 로 매핑함
    public ResponseEntity<?> insertBoard(HttpServletRequest request, @RequestBody BoardDto boardDto,
        ...
        // 참고 : 공지사항인 경우 Role 이 ADMIN인지 확인도 필요함
        // String adminYN = jwtUtil.getAdminFromToken(token); // 토큰에서 관리자 여부 추출함

//        if(adminYN.equals("N")) { // 관리자가 아니라면 접근을 거부함
//            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("관리자만 사용할 수 있는 서비스입니다.");
//        }

...
public class ReissueController {
    ...
    // access 토큰 재발급 요청용 컨트롤러 (단, 리프래쉬 토큰이 정상 상태일때만 재발급 처리함)
    ...
    public ResponseEntity<?> reissue(HttpServletRequest request, HttpServletResponse response) {
        ...
        // 사용자 정보 조회
        MemberDto memberDto = userService.selectMember(userId);
        if (memberDto == null) {
        ...
        if (refreshTokenOptional.isEmpty() || !refreshTokenOptional.get().getMember().toDto().equals(memberDto)) {
        ...
        // 리프래쉬 토큰이 만료되지 않은 상태이면, 새로운 액세스 토큰 생성
